...
(function(global) {
  ...
  const DouBaoPlatform = {
    ...
    async extractData() {
      console.log('[AI Export] Extracting DouBao data...');
    ...
    async extractDataFromDOM() {
      console.log('[AI Export] Extracting DouBao data from DOM...');
      ...
      sortedElements.forEach((item, index) => {
        try {
          ...
          console.error('[AI Export] Failed to extract DouBao message:', err);
        }
      });
    ...
    extractMessagesGeneric() {
      const messages = [];
      const mainEl = document.querySelector('[role="main"], [class*="main"], [class*="chat-container"], [class*="conversation-container"], [class*="dialog-container"]');

      if (!mainEl) return messages;

      const allBlocks = mainEl.querySelectorAll('div[class*="message"], div[class*="chat-item"], div[class*="conversation"], div[class*="bubble"], [class*="item"], [class*="dialog-item"]');
      let lastRole = null;
    ...
    async extractConversationList() {
      console.log('[AI Export] Extracting DouBao conversation list...');
      ...
      for (const selector of selectors) {
        ...
        elements.forEach(el => {
          ...
          const idMatch = href.match(/\/(?:chat|conversation|c|session|dialog)\/([a-zA-Z0-9-_]+)/);
          const id = idMatch ? idMatch[1] : null;

          if (id || href) {
            const title = el.textContent?.trim() || '';
            if (title) {
              const convId = id || `doubao-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
              if (!conversations.find(c => c.id === convId)) {
                conversations.push({
                  id: convId,
                  title,
                  url: href.startsWith('http') ? href : `https://www.doubao.com${href}`,
                  platform: 'doubao',
                });
              }
            }
    ...
    parseApiResponse(data) {
      ...
      if (data.result || data.content || data.text || data.output || data.answer) {
        const content = data.result || data.content || data.text || data.output || data.answer || '';
