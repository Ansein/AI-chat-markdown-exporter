...
(function(global) {
  ...
  const TongYiPlatform = {
    ...
    async extractData() {
      console.log('[AI Export] Extracting TongYi data...');
    ...
    async extractDataFromDOM() {
      console.log('[AI Export] Extracting TongYi data from DOM...');
      ...
      sortedElements.forEach((item, index) => {
        try {
          ...
          console.error('[AI Export] Failed to extract TongYi message:', err);
        }
      });
    ...
    async extractConversationList() {
      console.log('[AI Export] Extracting TongYi conversation list...');
      ...
      for (const selector of selectors) {
        ...
        elements.forEach(el => {
          ...
          if (id || href) {
            ...
            if (title) {
              const convId = id || `tongyi-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
              if (!conversations.find(c => c.id === convId)) {
                conversations.push({
                  id: convId,
                  title,
                  url: href.startsWith('http') ? href : `https://tongyi.aliyun.com${href}`,
                  platform: 'tongyi',
                });
              }
            }
    ...
    parseApiResponse(data) {
      ...
      if (data.result || data.content || data.text || data.output) {
        const content = data.result || data.content || data.text || data.output || '';
