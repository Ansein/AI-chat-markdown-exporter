...
(function(global) {
  ...
  const YiYanPlatform = {
    ...
    async extractData() {
      console.log('[AI Export] Extracting YiYan data...');
    ...
    async extractDataFromDOM() {
      console.log('[AI Export] Extracting YiYan data from DOM...');
      ...
      sortedElements.forEach((item, index) => {
        try {
          ...
          console.error('[AI Export] Failed to extract YiYan message:', err);
        }
      });
    ...
    extractMessagesGeneric() {
      const messages = [];
      const mainEl = document.querySelector('[role="main"], [class*="main"], [class*="chat-container"]');

      if (!mainEl) return messages;

      const allBlocks = mainEl.querySelectorAll('div[class*="message"], div[class*="chat-item"], div[class*="conversation"], div[class*="bubble"]');
      let lastRole = null;
    ...
    isUserMessage(el) {
      const userClasses = ['user', 'my', 'question', 'left'];
      const assistantClasses = ['ai', 'bot', 'assistant', 'answer', 'response', 'right'];
    ...
    extractCodeBlocks(element) {
      ...
      element.querySelectorAll('pre, code, [class*="code"]').forEach((el, index) => {
        blocks.push({
          index: index + 1,
          language: el.getAttribute('data-lang') || el.getAttribute('data-language') || 'text',
    ...
    async extractConversationList() {
      console.log('[AI Export] Extracting YiYan conversation list...');
      ...
      for (const selector of selectors) {
        ...
        elements.forEach(el => {
          ...
          const idMatch = href.match(/\/(?:chat|conversation|c)\/([a-zA-Z0-9-_]+)/);
          const id = idMatch ? idMatch[1] : null;

          if (id || href) {
            const title = el.textContent?.trim() || '';
            if (title) {
              const convId = id || `yiyan-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
              if (!conversations.find(c => c.id === convId)) {
                conversations.push({
                  id: convId,
                  title,
                  url: href.startsWith('http') ? href : `https://yiyan.baidu.com${href}`,
                  platform: 'yiyan',
                });
              }
            }
    ...
    parseApiResponse(data) {
      ...
      return {
        messages,
        metadata: {
          conversationId: data.conversationId || data.conversation_id || data.id,
          title: data.title,
          model: data.model,
        },
      };
